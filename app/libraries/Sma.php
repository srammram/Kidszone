...
class Sma
{
    ...
    public function formatMoney($number, $symnol = NULL)
    {
        ...
        public function exchangeformatMoney($number, $symnol = NULL)
    {
        $symbol = $symnol ? $symnol : $this->Settings->symbol;
        if ($this->Settings->sac) {
            return ((($this->Settings->display_symbol == 1 || $symbol) && $this->Settings->display_symbol != 2) ? $symbol : '') .
        ...
        $decimals = $this->Settings->exchange_decimals;
        $ts = $this->Settings->thousands_sep == '0' ? ' ' : $this->Settings->thousands_sep;
        $ds = $this->Settings->decimals_sep;
        return ((($this->Settings->display_symbol == 1 || $symbol && $number != 0) && $this->Settings->display_symbol != 2) ? $symbol : '') .
        number_format($number, $decimals, $ds, $ts) .
    ...
    public function actionPermissions($action = null, $module = null)
    {
        if ($this->Owner || $this->Admin) {
            ...
            if (!$module) {
                if($this->Settings->qsr ==1 &&  $current_url = $this->uri->segment(2) == 'qsr' ){
                    if(substr_replace($module, 'pos', 0)){
                        $this->m = 'pos';
                        $module = $this->m;
                    }else{
                        $module = $this->m;
            ...
            // var_dump($this->GP[$module . '-' . $action]);die;
    ...
    public function qrcode($type = 'text', $text = 'http://srampos.com', $size = 2, $level = 'H', $sq = null)
    {
	...
	public function paid_opts_front($paid_by = null, $purchase = false, $empty_opt = false)
    {
        $opts = '';
        if ($empty_opt) {
            $opts .= '<option value="">'.lang('select').'</option>';
        ...
        foreach($paymentMethods as $k => $method){
            $opts .= '<option value="'.$method->payment_type.'"'.($paid_by && $paid_by == $method->payment_type ? ' selected="selected"' : '').'>'.lang($method->payment_type).'</option>';
        }
    ...
    function re_direct_url($a) {

        if($a!=1){
            $this->session->set_flashdata('message', lang("access denied"));
            admin_redirect("welcome/index");
        }
    }
}
