...
class Kidsapi_demo extends REST_Controller {
	...
	function __construct() {
		...
		$this->load->api_model('kids_api_demo');
	...
	public function register_post(){
		...
		//if ($this->form_validation->run() == true) {
				...
				$res = $this->kids_api_demo->insertRegister($data);
				if($res == TRUE){
					$result = array( 'status'=> 1, 'message'=> 'Register Success --'.$res);
				}else{
					$result = array( 'status'=> 0, 'message'=> 'Not Register --'.$res);
				}
		/*} else {
			$error = $this->form_validation->error_array();
			 foreach($error as $key => $val){
