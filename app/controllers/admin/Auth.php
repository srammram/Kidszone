...
class Auth extends MY_Controller
{
    ...
    function getUsers()
    {
        ...
        <ul class="dropdown-menu pull-right" role="menu">
            <li><a href="' . admin_url('auth/profile/$1') . '"><i class="fa fa-edit"></i> ' . lang('profile_edit') . '</a></li>';
        if ($this->Settings->procurment == 1) {
            $action .= '<li><a href="' . admin_url('system_settings/permissions/$2/$1') . '"><i class="fa fa-bars"></i> '
                . lang('user_permission') . '</a></li>';
        }
		
        //$this->db->dbprefix('users').".id as id, user_number, first_name,".$this->db->dbprefix('warehouses').".name branch,".$this->db->dbprefix('users').".email as email, company, award_points, " . $this->db->dbprefix('groups') . ".name,
        $this->sma->checkPermissions('users',true);
        $this->load->library('datatables');
        $this->datatables
            ->select("'sno',".$this->db->dbprefix('users').".id as id, user_number,first_name,".$this->db->dbprefix('warehouses').".name as branch,".$this->db->dbprefix('users').".email as email,company, award_points, " . $this->db->dbprefix('groups') . ".name, active,".$this->db->dbprefix('users').".group_id as groupid")
            ->from("users")
	    ...
	    ->edit_column('groupid', $action, 'id,groupid');
            //->add_column("Actions", $action, "id,group_id");
    ...
    function login($m = NULL)
    {
		...
		if($this->data['user_stores'] && count($this->data['user_stores'])==1){
		   ...
		   admin_redirect($referrer);
		}else if($this->data['user_stores'] && count($this->data['user_stores'])>1){
		    $sessionData =  $this->session->userdata();
		    $sessionData['assigned_stores'] = $this->data['user_stores'];
		    $this->session->set_userdata($sessionData);
		    admin_redirect($referrer);
		}else{
		    $udata['status'] = "logged_out";
		    $this->site->updateLoginStatus($udata);
    ...
    function create_user()
    {
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $additional_data, $active, $notify,$stores_access)) {
...
//echo '<pre>';print_R($stores_access);exit;
        }
        if ($this->form_validation->run() === TRUE && $this->ion_auth->update($user->id, $data,array(),$stores_access)) {
    ...
    function user_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {  $this->sma->checkPermissions('delete',true);
                ...
                if ($this->input->post('form_action') == 'export_excel') {$this->sma->checkPermissions('excel',true);
	...
	$this->session->set_userdata('warehouse_id',$storeid);
	$this->session->unset_userdata('assigned_stores');
	echo 1;
    }
