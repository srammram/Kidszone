...
class Auth_model extends CI_Model
{
    ...
    public function register($username, $password, $email, $additional_data = array(), $active = FALSE,$stores_access=array())
    {
        ...
        if($id && !empty($stores_access)){
            foreach($stores_access as $k => $store){ echo $store;
                $store_acces_data['store_id'] = $store;
                $store_acces_data['user_id'] = $id;
                $this->db->insert('user_store_access', $store_acces_data);
            }
        }
        /*if(!empty($groups)) {
            //add to groups
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, last_ip_address, avatar, gender, group_id, first_name, last_name, company, phone')
    ...
    public function update($id, array $data, $upgs = array(),$stores_access=array())
    {
        ...
        if($user->id && !empty($stores_access)){
            $this->db->where('user_id',$user->id);
            $this->db->delete('user_store_access');
            foreach($stores_access as $k => $store){ echo $store;
                $store_acces_data['store_id'] = $store;
                $store_acces_data['user_id'] = $id;
                $this->db->insert('user_store_access', $store_acces_data);
