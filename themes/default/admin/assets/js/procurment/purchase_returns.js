...
$('#pi_discount').focus(function () {
   ...
   $('#add-purchase-return input[type="submit"]').click(function(e){
...
function loadItems() {
	...
	var $total_no_qty=0;
	    ...
	    tr_html +='<td><input class="form-control text-center received_quantity numberonly" name="received_quantity[]" type="text"  value="' + formatQuantity2(item_received_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '"></td>';
			...
			$batch_readonly= 'readonly';//(item.row.batch_required==0)?' readonly':'';
			$mfg_readonly = 'readonly';//(item.row.expiry_date_required==0)?' readonly':'';
			$item_dis_readonly = 'readonly';
		...
		$expiry_type ='';
			...
			$f_checked = (item.row.item_dis_type=="f")?'checked="checked"':'';
                tr_html += '<td class="text-right"><div style="width:100px !important"><input class="form-control text-right input-sm rdiscount numberonly" name="item_dis[]" '+$item_dis_readonly+' maxlength="20" type="text" id="discount_' + row_no + '" value="' + (item_ds != 0 ? item_ds :  '') + '" style="width:60px !important"><input type="radio" class="item-dis-type skip" name="item_dis_type['+$row_cnt+']" id="item-dis-type-p-'+$row_cnt+'" value="p" '+$p_checked+'><label for="item-dis-type-p-'+$row_cnt+'">%</label>'+'<input type="radio" class="item-dis-type skip" name="item_dis_type['+$row_cnt+']" id="item-dis-type-f-'+$row_cnt+'" value="f" '+$f_checked+'><label for="item-dis-type-f-'+$row_cnt+'">F</label></div></td>';
    ...
    if (pi_items[item_id]) {
		...
		bootbox.confirm('This item is already added. Do you want to add it again?', function (result) {
			...
			if (result) {
				...
				var received_qty = parseFloat(pi_items[item_id].row.r_qty) + 0;
				pi_items[item_id].row.base_quantity = new_qty;
				if(pi_items[item_id].row.unit != pi_items[item_id].row.base_unit) {
					$.each(pi_items[item_id].units, function(){
						if (this.id == pi_items[item_id].row.unit) {
				...
				pi_items[item_id].row.qty = new_qty;
				pi_items[item_id].row.r_qty = received_qty;
